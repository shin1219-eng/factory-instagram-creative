...
const SPECTOR_PATH = process.env.SPECTOR_PATH || path.join(__dirname, "vendor", "spector.bundle.js");
const SPECTOR_URL = process.env.SPECTOR_BUNDLE_URL || "https://spector.babylonjs.com/spector.bundle.js";

const HEADLESS = process.env.HEADLESS === "1";
const WAIT_MS = Number(process.env.WAIT_MS || 7000);
const CAPTURE_TIMEOUT_MS = Number(process.env.CAPTURE_TIMEOUT_MS || 15000);
...
async function ensureSpectorBundle() {
  if (fs.existsSync(SPECTOR_PATH)) return;
...
async function captureUrl(url) {
  ...
  try {
    ...
    await ensureSpectorBundle();
    await page.addScriptTag({ path: SPECTOR_PATH });

    const captureResult = await page.evaluate(async (timeoutMs) => {
    ...
    await page.screenshot({ path: path.join(shotsDir, "hero.png"), fullPage: false });
